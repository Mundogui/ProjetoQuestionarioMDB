...
def distribution_chart():
    ...
    labels = [
            ...
            'EscProjE6', 'ColetDadosP7', 'ProcAutomP8', 'ProcPersExP9', 'ProcInfoP10',
...
def radar_chart(setor):
    ...
    cursor.execute(f"SELECT * FROM respostas WHERE setor = ?", (setor,))
...
#----------------------estatisticas Mediana,Variancia e Desvio Padrao--------------------------------
...
def calculate_variance(data):
    """
    Calcula a variância de uma lista de dados.
    """
    # Verifique se há pelo menos dois pontos no conjunto de dados
    if len(data) < 2:
        return None  # ou algum valor padrão, dependendo do seu caso
...
def calculate_summary_statistics(responses):
    """
    Calcula a mediana, variância e desvio padrão para cada questão.
    """
    # Inicialize listas para armazenar as pontuações de cada questão
    question_scores = [[] for _ in range(24)]

    # Processar as respostas e extrair as pontuações de cada questão
    for response in responses:
        for i in range(24):
            question_scores[i].append(response[i + 5])  # As respostas começam no índice 5 da tupla

    # Inicialize listas para armazenar as estatísticas resumidas
    median_values = []
    variance_values = []
    std_deviation_values = []

    # Calcular as estatísticas para cada questão apenas se houver dados suficientes
    for scores in question_scores:
        if len(scores) >= 2:
            median_values.append(calculate_median(scores))
            variance_values.append(calculate_variance(scores))
            std_deviation_values.append(calculate_std_deviation(scores))
        else:
            # Adicione um valor padrão ou mensagem indicando que não há dados suficientes
            median_values.append(None)
            variance_values.append(None)
            std_deviation_values.append(None)

    # Retorna as estatísticas calculadas
    return median_values, variance_values, std_deviation_values

# Rota para a página admin (admin.html)
@app.route('/estatistica')
def estatistica_page():
    # Conectar ao banco de dados usando o bloco with para garantir que a conexão seja fechada corretamente
    with sqlite3.connect('respostas.db') as conn:
        cursor = conn.cursor()

        # Obter todas as respostas do banco de dados
        responses = cursor.execute('SELECT * FROM respostas')
        responses = cursor.fetchall()

    # Calcule as estatísticas resumidas
    median_values, variance_values, std_deviation_values = calculate_summary_statistics(responses)

    # Renderize a página estatistica.html com os valores das estatísticas
...
def calcular_mad(data):
    median = np.median(data)
    mad_constant = 1.4826
    mad = mad_constant * np.median(np.abs(data - median))
    return mad

def verificar_outliers(data, medida):
    mad = calcular_mad(data)
    if np.max(data) > mad:
        return f"{medida} está fora do comum em relação à MAD."
    else:
        return f"{medida} está dentro do esperado em relação à MAD."

def plotar_grafico_dispersao(data):
    median = np.median(data)
    mad = calcular_mad(data)
    lower_extreme = median - 3 * mad
    upper_extreme = median + 3 * mad

    plt.scatter(range(1, len(data) + 1), data, label='Dados')
    plt.axhline(y=median, color='r', linestyle='-', label='Mediana')
    plt.axhline(y=lower_extreme, color='g', linestyle='--', label='Limite Inferior (3 MAD)')
    plt.axhline(y=upper_extreme, color='b', linestyle='--', label='Limite Superior (3 MAD)')

    plt.title('Gráfico de Dispersão com Linhas de Extremos')
    plt.xlabel('Amostra')
    plt.ylabel('Valor')
    plt.legend()

    # Convertendo o gráfico para uma imagem
    img = BytesIO()
...
def processar_dados():
    ...
    # Verificar variância e desvio padrão
    variancia_outcome = verificar_outliers(dados, "Variância")
    desvio_padrao_outcome = verificar_outliers(dados, "Desvio Padrão")

    # Plotar gráfico de dispersão e obter a URL da imagem
    plot_url = plotar_grafico_dispersao(dados)

    return render_template('resultado.html', variancia_outcome=variancia_outcome, desvio_padrao_outcome=desvio_padrao_outcome, plot_url=plot_url)
